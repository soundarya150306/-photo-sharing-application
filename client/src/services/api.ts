...
export const api = {
  ...
  uploadPhotos: async (eventId: string, formData: FormData, onUploadProgress?: (progressEvent: any) => void) => {
    try {
      return await apiClient.post(`/events/${eventId}/photos`, formData, {
      ...
      photosMap[eventId] = currentPhotos;
  ...
  updateSelection: async (eventId: string, photoIds: string[], isSelected: boolean) => {
    try {
      ...
      return mockResponse({
        success: true,
        message: `Updated selection for ${photoIds.length} photos`,
  ...
  deletePhoto: async (eventId: string, photoId: string) => {
    try {
      ...
      const photosMap = getMockPhotos();
      const current = photosMap[eventId] || [];
      photosMap[eventId] = current.filter((p) => p.id !== photoId);
      saveMockPhotos(photosMap);
    ...
    try {
      ...
      if (ev) {
        ev.gallery = {
          id: `gal-${Date.now()}`,
          eventId: ev.id,
          slug: data.slug || ev.gallery?.slug || 'arjun-priya-wedding',
  ...
  getPublicInfo: async (slug: string) => {
    try {
      ...
      const events = getMockEvents();
      const ev = events.find((e) => e.gallery?.slug === slug) || events[0];
      return mockResponse({
        success: true,
        data: {
          gallery: {
            slug: ev.gallery?.slug || slug,
            ...
            totalSelectedPhotos: ev.stats?.selectedPhotos || 4,
            requiresPin: true,
          },
        },
      });
  ...
  getPublicPhotos: async (slug: string, guestToken: string) => {
    try {
      ...
      const ev = events.find((e) => e.gallery?.slug === slug) || events[0];
      const photosMap = getMockPhotos();
      const photos = (photosMap[ev.id] || photosMap['0deb59f0-46b4-4231-b694-f14d982172f1'] || []).filter(
        (p) => p.isSelected
      );
